...
// 从 AI 响应中提取图谱数据
function extractGraphData(content) {
    try {
        // 方式 1: 尝试使用明确的标记提取
        const markerRegex = /\[GRAPH_DATA_START\]\s*```json\s*([\s\S]*?)\s*```\s*\[GRAPH_DATA_END\]/;
        const markerMatch = markerRegex.exec(content);
        
        if (markerMatch) {
            const jsonStr = markerMatch[1];
            const data = JSON.parse(jsonStr);
            
            if (data.graph && data.graph.nodes && data.graph.edges) {
                return {
                    graph: data.graph,
                    chain_name: data.industry || data.chain_name,
                    stats: data.stats
                };
            }
        }
        
        // 方式 2: 尝试匹配 JSON 格式的图谱数据（兼容旧格式）
        ...
        // 不是 JSON 格式，忽略
        console.debug('图谱数据提取失败:', e.message);
    }
